...
parser.add_argument('--ckpt_dir', default='./save_models/pretrained', type=str, help='ckpt path')
parser.add_argument('--pretrain', default='epoch-150-pr-0.486-kappa-0.809.pth.tar', type=str, help='path to the pretrained model')
...
def val(data_loader, model, class_weights_t):
    ...
    with torch.no_grad():
        for i, (image, targets) in enumerate(data_loader):
            ...
            output_t, output_t_feat = model(image)
            val_feat.append(output_t_feat)
    ...
    return val_feat.cpu().numpy(), val_label.cpu().numpy()
...
def main(args):
    ...
    val_feat, val_label = val(val_loader_t, model, class_weights_t)
