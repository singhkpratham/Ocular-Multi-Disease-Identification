...
class _UpBlock(nn.Sequential):
    """(Unsampling -> conv)"""
    def __init__(self, in_channels, out_channels, norm_type='Unknown', bilinear=True):
        super(_UpBlock, self).__init__()
        if bilinear:
            self.add_module('conv1', nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False))
    ...
    def forward(self, x1, x2):
        ...
        return super(_UpBlock, self).forward(x)
